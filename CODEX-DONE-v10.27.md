...
★ 收尾复跑时这条套件曾报 **35/36**，唯一那条红是**假红**（见「坑 4」的像素上界），
改成相对判据后复跑 **36/36，失败 0**。
...
### 坑 4：布局类断言写死像素上界 ⇒ 早晚假红

收尾复跑实拍时撞到：断言「折叠的区块只剩标题行」写成 `h > 0 && h < 60`，
实测折叠标题行 **66px** ⇒ 报红，但**语义上它完全正确**（66px 就是一行标题的高度）。
写死的 `60` 是**目测猜的**，而标题行高会随字号 / 内边距 / 文本换行漂移。

**修法：改成相对判据** —— `h > 0 && h < openSection.h / 3`
（`>0` 守「仍可见可点」，`< 展开态/3` 守「没被整块铺开」），并在输出里**同时打印两者**，
下次再红一眼就能分辨「布局真变了」还是「数据变了」。

> ⇒ **凡「某元素应该比较小 / 矮 / 窄」的断言，一律对照同一时刻的参照物，不写绝对数。**

### 坑 5：截图只证明「拍进去的那一块」，不能替代计数断言

默认视图截图里「本体 22 帖」只看得见 7 行，一度怀疑漏行。实测是
**`.dlpop-box` 限高 80vh、body 可滚动**，第 8~12 行与「展开全部」在折线以下
（展开态实测 **851px**，折叠标题行 **66px**）。
⇒ **判「有没有漏」要用计数断言**（12 行来自 12 个不同 `postId`），**不要靠看图数行**。
