...
>
> 基线 v10.29 · 本版未发布（LIVE 仍为 v10.28）
...
**本版未发布**。LIVE 仍为 v10.28（`gamehub-agg-v4`）。
发布需**逐轮重新拿授权**；发布后必须跑 `tools/verify-online.js`（判新版**比 md5**，
HTTP 200 区分不出新旧）并更新 `verify-online.js` 的 `MUST` 串（曾停在 v10.24 ⇒ 发不发都绿）。
