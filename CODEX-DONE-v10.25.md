...
## 十二、收尾：启动器三件套 + 推送脚本两处修复（发布后追加）

### 12.1 启动器修掉一个「静默失效很久」的回退分支

| # | 现象 | 根因 | 修法 |
|---|---|---|---|
| 1 | `.cmd` / `.vbs` 的 managed node 回退**永远走不到** | 路径写死 `…\node\versions\22.22.2-2\node.exe`，而实际目录是 **`22.22.2-3`**（运行时目录名带**构建后缀，升级就变**） | 改成**扫描** `versions/` 取最新：`.cmd` 用 `dir /b /ad /o-n` 倒序、`.vbs` 取字典序最大 |
| 2 | 9-04 的旧 `启动聚合站.bat` 会**起出一串实例** | 路径 `C://Users//…//22.22.2//`（双斜杠 + 漏后缀）+ **无幂等启动** ⇒ 撞上 `server.js` 的 `listen(port+1, 30)` 兜底，一路 8124/8125… | 归档到 `_archived/launcher-v1-20260904.bat`（**不删**，可一句话恢复） |

**两套扫描算法都在测试里真跑了一遍**：`.cmd` 的 `dir /o-n` 倒序取首个 = `22.22.2-3`、
`.vbs` 的字典序最大 = `22.22.2-3`，**两者一致**且路径 `fs.existsSync` 为真。
这类「版本漂移」只有实跑才抓得住 —— 只查「字符串里有没有 `versions\22.`」是不够的。

### 12.2 新增「打开线上版.url」

`[InternetShortcut]` 指向 LIVE。**它的 URL 必须与 `tools/report.js` 的 `LINKS.LIVE` 相等**，
`.cmd` 横幅里的 `WEB=` 同理 —— 三条断言守着「换链接时漏改一处」（本项目的老病：
同一语义散在两处就是"改一处忘一处"的温床）。

### 12.3 新防线 `tools/test-launcher.js`（45 条）

守六族：幂等启动（端口正则**必须带尾随空格**，否则 `:8123` 命中 `:81230`）·
node 定位（**禁写死版本号** + 算法实跑）· 编码（`.cmd`/`.vbs`/stop 必须**纯 ASCII**）·
停止脚本（`tokens=5` + `taskkill /F` + 幂等）· 端口四处一致 · 链接三处同源 · 旧脚本已归档。

- **反证 5 个坏点各变红、零假绿**（`test-v1025-launcher-counterproof.js`）
- 已登记进 `run-all.js` 的 `SUITES` ⇒ 静态防线 **24 套 1573 / 0**

### 12.4 推送脚本 `_push-via-api.js` 的两处真缺陷（本次才暴露）

1. ★ **中文文件名**：Git 默认 `core.quotepath=true` 把非 ASCII 路径转义成
   `"\345\220…"` 带引号串 ⇒ `cat-file blob HEAD:<path>` 直接 `fatal: path does not exist`。
   **推了几十次都没事，因为提交里清一色 ASCII 路径**。修：两个 git 封装统一加
   `-c core.quotepath=false`。
2. ★ **`git diff --name-only` 看不到被删的旧路径**：`git mv` 之后只列新名 ⇒
   重建 tree 时 `base_tree` 里的旧路径还在 ⇒ 新 tree 多一个文件 ⇒
   判据「**远端 tree == 本地 tree**」当场拦住。
   **这是护栏立功**（真推上去仓库里就多一份删不掉的文件），不是护栏坏了。
   修：改用 `--name-status -M`，删除项在 Git Data API 里用 **`sha: null`** 表达。

### 12.5 顺带

- 四个脚本（`.cmd`/`.vbs`/`stop`/`.url`）**全部转回 CRLF** —— `Edit`/`Write` 改写后会留 LF，
  而 `git add` 只给一句警告，静默留在工作区（用户双击的正是这份）。
- `MEMORY.md` 从 3190 → **2679 字**（`test-report.js` 的 3000 上限先前挂了）。
