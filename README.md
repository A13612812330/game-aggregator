<!-- ===================== v10.11 ~ v10.23 汇总 ===================== -->
## ★ v10.23 增量（2026-09-20，最新）—— 跨源跳转直达详情页
...
静态 **21 套 / 1388 条 / 0 失败**（新增 `tools/test-twin.js` 69 条）·
实拍 `tools/preview-v1023.js` **17/17**（含 `elementFromPoint` 验「真能点到」）·
反证 `tools/_counterproof-v1023.js` **4/4 打坏即变红**。
★ 反证当场抓出一条**假绿**（断言被 CSS 注释里引用的同一串文字命中 ⇒ 改行首锚定），
并修掉一条**假红**（「不残留裸 CSS 文本」误伤 `<style>` 内部的注释）。
...
```
node tools/run-all.js                → 21 套 / 1388 条 / 0 失败
