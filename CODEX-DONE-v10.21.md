...
发布完才想起一件事：**本项目每发一版就多一条同名 app + 多一个域名**，而平台**没有删除 app 的接口**
（sites 工具只有 `deploy` / `unpublish`），历史域名全都返回 **200 但内容停在旧版**。
于是「哪个域名还能用」每次都要手工查 —— 本次把它做成常驻脚本 `tools/audit-apps.js`。

### 7.1 实测结论（2026-09-18，比上一轮的说法更准）

| 域名 | HTTP | index.md5 | 实际停点 |
|---|---|---|---|
| **`gamehub-agg-v3`** | 200 | `cb2c07b3b4` | ✅ **与本地同版（含数据层）** ← 正式入口 |
| `gamehub-agg-v2` | 200 | `73a715a1a4` | 停在 **v10.18**（缺 `eg-nav`） |
| `gamehub-agg-join` | 200 | `55dcb8f7ff` | 停在 **v10.17**（`/api/device/fill-stats` 404） |
| `36aa37e9…` | 200 | `cb2c07b3b4` | ⚠️ **也已是 v10.21（含数据层）** |

★ **第 4 行推翻了本项目记忆里的旧说法**（原记「碰巧含 v10.18」）——它现在**与本地逐字节一致**。
但**不因此把它当入口**：它不在 `applications.yaml` 的正式登记里，**更新不保证跟随**；
哪天不再跟随，用户就会看到旧界面（这正是本项目踩过的那个坑）。⇒ 只当别名，对外仍用 v3。

### 7.2 三个 app 登记（清理用）

| # | appId | 建立时间（genie 证据） |
|---|---|---|
| 1 | `wbapp_k0YzBHCqnzzK9YrcG5Bru0` | 2026-09-18 02:18:39 |
| 2 | `wbapp_D5cUfPk1IwCJlI3eIgc96h` | 2026-09-18 03:08:11 |
| 3 | `wbapp_WNoF5maytwivZ60TUa6CPA` | 2026-09-18 08:22:53 ← **本次 v3** |

### 7.3 ★ 一个必须写下来的危险动作

**别用 sites 的 `unpublish` 去清旧链接**：它取的是「**同一本地目录的最新一次发布**」，
而三个 app 的 `localDir` 全都是同一个 `game-aggregator` ⇒ **会把 v3 一起下掉**。
旧 app 只能在「设置—数据管理—应用」里**手工删**。这条已写进脚本输出，也钉了断言。
（同族铁律：**破坏性动作要先问「它的作用域是什么」，别信工具名**。）

### 7.4 踩坑与反证（本工具自己出的两个错）

| # | 坑 | 后果 | 修法 |
|---|---|---|---|
| ① | 门控 `test` 按 JSON/text **分流**（`status===200 ? body : resp`） | 判据写的是 `(r) => r.status === 200`，收到 body ⇒ **恒假**，把**最新的 LIVE 判成「缺 v10.20」** | 统一返回 `{status,text,body}`，判据只吃这个对象 |
| ② | 命中第一个缺口就 `break` | 「停在 v10.18」被报成「尚未发布 v10.21」——**都对，但没用**，还掩盖真实停点 | 收集**全部**缺口，取**最旧**那个当停点 |

★ 两处都属「**看起来更严格、实则报反**」，与本项目 v10.20 修过的那个「报反」同类。
新增 `tools/test-audit-apps.js`（**28 条**）守住的正是这两个坑 + `GATES` 必须**由新到旧**
（顺序反了，「取最旧」就变成「取最新」，结论整个反过来）。

**反证**：把上面三处逐个打坏 → 断言**全部正确变红**（M1/M2/M3 各命中对应断言），还原后复跑全绿。
静态防线 **19 套**。
