...
> **数值权威性（2026-09-18 更新，v10.20 时）**：本文是**早期设计稿**，第 2/4/5/6 节的取色与尺寸
> 写下时较早，**一律以 `public/index.html` 的 `:root` 变量与实现为准**
> （当前主色 `--c-primary:#2E6BFF`，非旧稿的 `#5B5BD6`）。
>
> ⚠️ **第 4 节「筛选条 `.filter-bar`」已不是当前实现**（它写的是 v10.3 那套单行筛选条）：
>
> | 时间 | 改成了什么 |
> |---|---|
> | v10.5 | 拆成**三行** `.filter-row`（排序 / 容量 / 筛选各一行，各有行首标签 `.filter-lb`）|
> | v10.6+ | 开关状态标记改走**定宽伪元素** `.em-tg::before`（不进文案 → 盒宽恒定，切换不顶掉同排）|
> | v10.9 | 手游 / 修改器 / 云存档三条筛选条共拆成 **7 行 `.emu-bar-row`** |
