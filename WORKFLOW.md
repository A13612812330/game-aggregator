...
```bash
# 第一层：静态（前置闸 2 个 + 32 套 / 2183 条 / 必须 0 失败）
...
| ① 静态 | `tools/run-all.js` | **32 套 / 2183 条** | 秒级、无需人盯 | 每次改完 |
| ② 行为 | `test-emulator-page.js`（jsdom，含在 32 套内） | 119 条 | 需服务在 8123 | 每次改完 |
...
**当前阶段**：`ROADMAP.md` 第二节 **阶段一（P0）· 配置要求预热** —— 等用户授权。
（实测缺口 **17,424** 条 / 串行 **5.3h**、3 并发 ~1.8h；库里 95.4% 的条目有 appid，缓存只覆盖 3.9%。）

### ✅ 已完成（最近：2026-09-22）

| 项 | 结果 |
|---|---|
| **★ 建立 `ROADMAP.md`（功能全景 + 三阶段规划）** | 产品视角**唯一入口**：三页 + 详情抽屉 8 分区 + **15 个数据层库** + **9 个外部源** + 工程底座，全部数字**实测**（`_probe-inventory.js` / `_probe-gap.js`）。第二节给出三阶段（P0 预热 / P0-P1 匹配可信度 / P1-P2 体验收口）+ v11 主轴 + **三项待你决策**。⇒ 本文第七节同时改为**指针**，避免两份 Roadmap 漂移。★ 顺手纠正一处**口径错**：v10.32 记「带 appid **15,007** 条」，实测那只是「**封面 URL 能解析出**」的（78.9%）；另有 **3,134** 条只有顶层 `appid` 字段 ⇒ **真实可预热总量 18,131 = 95.4%**，缺口 **17,424** 条 ≈ 串行 **5.3h**（旧表述 4.6h 偏低）。**数字对了不等于口径对了。** |
