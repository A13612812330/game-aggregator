...
> 基线：**v10.39**（2026-09-24）· 服务 `localhost:8123`
...
| C | **发布**（覆盖线上） | 用户明确要求 | 攒够一版才发 | `https://gamehub-agg-v4.app.workbuddy.host/` |
...
★ **v10.38 起收进一条命令**：`node tools/daily-sync.js`（**9 步**固定顺序）。
`--dry` 只规划不执行 · `--skip=bh,saves` 跳步 · `--json` 出机器可读摘要。
自动化 prompt 里**只留这一条命令**，不再逐步下发。

9 步顺序（★ 顺序**不可换**，后一步依赖前一步产物）：

| 步 | 动作 | 产物 | 备注 |
|---|---|---|---|
| 1 | 服务体检（不可达则拉起） | — | 端口固定 8123；标 `critical`，失败即中止 |
| 2 | `POST /api/library/index/incr?pages=10` | 端游库增量 | XDGAME；409（有任务在跑）本轮跳过 |
| 3 | `POST /api/bh/refresh` + 轮询 state | BannerHub 社区配置库 | 12~60s，最多等 12min；**成败看 `lastOk` 不是 `ok`** |
| 4 | `python tools/build-phonecfg.py` | `data/phonecfg.json` | 读 `基础测试数据.xlsx`（**只读**）；缺文件时标 `blocked / NEED_XLSX`，不当失败 |
| 5 | `node tools/build-mobilehub.js` | `data/mobilehub.json` | ★ 必须在 3、4 之后 |
| 6 | `node tools/fetch-trainers.js` | `data/trainers.json` | GCM 公开接口；源站不可达时回退上次产物并**如实标注** |
| 7 | `node tools/build-saves.js` | `data/saves.json` | 需 `.cache/ludusavi-manifest.yaml`（17MB） |
| 8 | `POST /api/library/index/jidi` | 机地话题 | 409 则先 `waitIdle` 再重试；★ 分源条数只在 `bySource` 里（`idxState.jidi.total` 是**全库总量**，直接展示会得到误导数字） |
| 9 | ★ **配置要求预热**（v10.39 新增） | `data/steam-req.json` + **`data/spec-req.json`** | ① `build-steam-req.js --limit=500` 联网补缓存 ② **再** `build-spec-req.js` 重建派生索引 —— **缺第 ② 步就是白抓**（索引的输入是缓存产物）。上限 500 条/日（实测 1.08s/条 ≈ 9 分钟），顶到上限时摘要必须报「仍有积压，次日续补」 |

★ 第 9 步放在**最后**：它依赖前 8 步跑完后的最新端游库（新入库的游戏才有 appid 可抓）。
★ 它是全流程**唯一**要联网逐条请求的步骤 ⇒ 超时单给 **25 分钟**（`--timeout-steamreq=`），
其余步 3~12 分钟；预热超时时**仍要跑第 ②步**（分批落盘已有成果，不重建等于白抓）。

**约束**：只调接口与指定脚本，**不改任何源码、不删任何东西**；
三个 v10 索引（mobilehub / trainers / saves）由 `data/*.js` 按 mtime 惰性加载，
**脚本跑完自动生效，无需重启**。
★ **唯一例外**：第 9 步重建的 `data/spec-req.json` 走**进程内缓存**，
**换数据后必须重启服务**才生效（v10.37 实测：服务内存陈旧快照 `cached 4423` vs 磁盘 `16312`）。
...
② **任意偏移下不存在大段重复** —— 主力判据，**不依赖标题**，所以连「标题被吞进行尾」也能抓；
③ 标题**路径**唯一（不是标题文本唯一 —— 同名子节挂不同父节是合法的）。
★ 两个实现要点（都踩过）：**必须跳过围栏代码块**（否则 ```bash 里的 `# 注释` 会被当成标题，
WORKFLOW.md 曾被误算成 24 个一级标题）；**判据要按文件成对自检**（偏移重复 + 标题路径两条分工，
任一条失效另一条仍能红 —— 反证第 ⑥ 组就是打掉偏移门槛、验证标题判据兜得住）。
...
| `CODEX-DONE-vX.Y.md` | **逐版完整说明**（**39 份**，v10.1 → v10.39） | 每版 |
...
**当前阶段**：`ROADMAP.md` 第二节 —— **阶段一（P0）✅ 预热已收官**（v10.37：命中 16,200/17,329，缓存 cached 16,312 / withReq 16,111）· **阶段二 2-B 已收口**（v10.34 字段校准 + **v10.35 口径决策已落**）· **2-A / 2-C 已收口两轮**（v10.36 唯一真源 + 代际护栏 ⇒ 51.0% → 51.3%；**v10.39 二轮实测判定「不该改」**，见下）· **派生索引已按预热结果重建**（v10.37：`data/spec-req.json` Steam 官方原文 3.9% → **94.1%**）。★ **v10.38 已把每日维护收进一条命令**（`node tools/daily-sync.js`，单轮实测 28 秒，只回吐 ≤8KB 摘要）；★ **v10.39 把「配置要求」接成每日增量**（第 9 步，上限 500 条/日，积压 925 条次日续补）；★ 预热本身**已跑完**（无需再跑）；★ `spec-req.json` 换数据后**必须重启服务**才生效（v10.37 实测：服务内存陈旧快照 `cached 4423` vs 磁盘 `16312`）。
...
| 项 | 结果 |
|---|---|
| **★ v10.39：把「结论依据」摊开 + 配置要求接上每日增量 + 一份文档事故** | ① **P0-1 口径**：「游戏没标 DX ⇒ 该维度 skip ⇒ 仍判流畅」原样保留（没标就一律不降级），**改的是「把依据摊开」**：`data/spec-match.js` 给每个 `skip` 标**责任侧**（`side:'game'` = 游戏数据没标 / `side:'profile'` = 我方配置没标），`judge()` 新增 `unstated` + `basis`，stats 同步加 `unstated` 分布；前端挂紫色 chip「未声明：图形接口」+ 「本款结论由 … 判定」。实测影响面 **6,585 款（17,120 的 38.5%）**、接口 `stats.unstated.dx = 6,619`（两口径互证）。★ **自己踩的坑**：首版用 `dims.filter(d => d.state === 'skip')` ⇒ 把 `arch` 也算进去（`unstated=["arch","dx","storage"]`），前端会报「未声明：架构」，而 arch 的 skip 是**配置侧**没标 ⇒ 必须按 `side` 分流。② **P1-1 匹配率二轮 = 判定「不改」**：抽 40 条 C 类，**约 40% 是 exe 残留/编码损坏的垃圾查询名**（`AssassinsCreedIIGame` / `Ragnar k` / `LEGOMARVELAvengers DX11`），匹配上反而错；真错配只有 2~3 条、而改护栏会**误杀 12 条对的** ⇒ 51.2% 已接近数据源天花板，**瓶颈在查询名质量不在匹配器**。③ **P1-2 第 9 步**：`--dry` 拦住全部 8 个写步骤、三个产物 mtime **零变化**；实测积压 **925 条**（抽跑 60 条 **100% 命中**、miss 0 / err 0）· 1.08s/条 · 索引重建 1.3s（17,177 款，DX 10,535 → 10,567）。静态 **38 套 / 2703 条**（新增 `test-doc-structure.js` **145 条** · `test-daily-sync.js` 101 → **128 条**）· 反证 **6/6**（含**真实事故回放**：把那个 752 行污染版放回去，闸必须在「偏移重复」与「标题路径」上变红）。④ ★ **顺手发现并修掉一份文档事故**：v10.36 提交把**整份 `WORKFLOW.md`** 插进了「已完成」表那条 v10.36 长行的**中间** —— 752 行里 336 行是副本，表行被劈成两半、**连文档标题都被挤进了表行末尾**。该状态**在仓库里活了 3 个版本**（v10.36 / v10.37 / v10.38）无人发现 ⇒ 重建 416 行（`git diff` = **1 insertion / 337 deletions**），并新增**文档结构闸**（见第四节「v10.39 新增」）+ 留下偏移比对探针 `_probe-workflow-dup.js`（锚点失配即报错，不猜位置）。 |
| **★ 每日维护收进一条命令（v10.38）** | 原先自动化 prompt 逐步下发 10 个动作 ⇒ 收进 `tools/daily-sync.js`（单轮实测 **28 秒**、只回吐 ≤8KB 摘要，`--dry` / `--skip=` / `--json` 三个开关）。★ **踩坑**：收尾报告第 ② 项「✅ 已是最新」是**假绿** —— 只比前端 `index.html` md5 **判不了 `data/**` 的改动**（服务端改动的版本前端字节完全不变）⇒ 加**服务端口径指纹**判据；判「线上是不是最新」= **前端 md5 + 服务端指纹两条都过**。 |
| **★ 配置要求预热收官（v10.37）** | 命中 **16,200 / 17,329（93.5%）**：缓存 1.04 MB → **5.92 MB**、限流 144 次自动退避、后台 **9h35m42s**。★ **派生索引必须重建才算落地**：`data/spec-req.json` 的 Steam 官方原文 **3.9% → 94.1%**、DX 标注 +2,223、六字段零退化。★ **校正一条旧认识**：`spec-req.json` 换数据后**必须重启服务**（v10.37 实测服务内存陈旧快照 `cached 4423` vs 磁盘 `16312`）—— 原记「跑完不必重启」**只对 `steam-req.json` 成立**。 |
| **★ 2-A + 2-C 跨源名字匹配（v10.36）** | 承接阶段二 **2-A（收朴素子串假阳性）+ 2-C（手游中心匹配率）**，用户决策「一起做」。★ **先量再改**：改前先建三桶分类探针（`_probe-match-audit.js`），把「未匹配」拆成 **Z 残渣名 / A 库中确无 / B 护栏该拦 / C 匹配器漏**，**只有 C 类可修**；`ROADMAP.md` 的「473 条假阳性」经重测**已过期**。真实症状是两个独立问题：① 3 个文件各写一份归一化、**且都漏剥 `™ ® ©`**（无唯一真源 ⇒ 漂移源，都不报错）② 数字/代际零校验（`CoD MW2 2009` 挂**使命召唤16**、`GTA 5` 挂**增强版**、`SF6` 挂**虚拟机版**）。**四处改动**：① 新建 `data/name-normalize.js` 唯一真源（断言用 `pc.normKey === NN.normKey` 比**函数同一性**）② **取数必须在 `normKey` 之前**（归一化吃掉空格 ⇒ `…warfare22009` 切出 `[2200,9]` ⇒ **正确匹配反被判冲突**，误杀 4 条）③ 代际护栏挂满**每一条**返回通道（`build-mobilehub` 5 处 / `phonecfg` 3 处；漏挂的失败形态是「改完变差了一点点」、**无任何报错**）④ 尾缀剥离 + 14 个 CJK 版本词。★ **两处「看着该改、实测不该改」**：前缀阈值 `>=8` → `10` 实测**丢 18 条正确匹配、只换回 1 条假阳性** ⇒ 保持 8；`RELEASE_SUFFIX` 加 `\s*application\s* ⇒ 与 `stripRelease` 终止条件**死循环**（`DEMO 版` → `DEMO` → 空）⇒ **判空放在循环之后** + `>=3` 长度闸。**实测**：手游中心 3,203/1,635 → **3,211/1,646**（净 +11）· 匹配率 51.0% → **51.3%**（剔残渣 55.6% → **55.9%**）· 可修 C 151 → **145** · 已匹配侧**硬冲突 3 → 1** · 实测库 651/1,025 → **653/1,025**。**逐条 diff**：① +14 ② −8（从错配变未匹配 —— `The Sims 1 - Legacy Collection` 原先挂 **The Sims™ 4 自然奇境资料片**）③ 改目标 3（`CoD MW2 2009` → 6（2009）/ `GTA 5` → 传承版 / `SF6` → 正式版）④ 只在基线 11 / 只在当前 19（**合并口径变化，非匹配器行为** ⇒ 净 +11 ≠ 14−8 的原因）。静态 **36 套 / 2415 条**（`test-v1036.js` 65 条）· 反证 **14/14** · 共享守卫**已改为自动发现**（实测 10 个共享模块）· 接口 12 连全对（含 `Tomb Raider` **反向护栏未误杀**）。★ **反证踩坑**：往已有 `const { normKey } = require(...)` 的文件里**插** `function normKey` ⇒ 重复声明 ⇒ SyntaxError ⇒ **套件整个崩掉** ⇒ 只能报 `UNVERIFIED` ⇒ 正确形态 = 把 `require` **换成**同名本地实现。**「崩了」不等于「抓住了」**。 |
...
| **★ 发布三坑（v10.22 一次全踩）** | ① **判据不是 HTTP 200** —— v2/join/v3 **三个域名全 200 却全是旧版**，只能比 `index.html` md5（★ v10.38 补：只比它还**不够** —— 改 `data/**` 的版本前端字节不变，单判据会假绿；判「线上是不是最新」= **前端 md5 + 服务端口径指纹两条都过**，见 `PITFALLS.md` 第十四节第 4 条，工具已落在 `report.js`） ② **LIVE 不跟随本地改动**（放临时文件探远端 → 404）⇒ 它是**快照**，改完本地**不会**自动上线 ③ 未登记的平台域名**不稳**（⚠️ ④ 之④见上：覆盖是否被拒**与环境相关**，不是永久结论） |
