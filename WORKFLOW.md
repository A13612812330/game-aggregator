...
```bash
# 第一层：静态（前置闸 2 个 + 37 套 / 2531 条 / 必须 0 失败）
...
| ① 静态 | `tools/run-all.js` | **37 套 / 2531 条** | 秒级、无需人盯 | 每次改完 |
| ② 行为 | `test-emulator-page.js`（jsdom，含在 37 套内） | 119 条 | 需服务在 8123 | 每次改完 |
...
**当前阶段**：`ROADMAP.md` 第二节 —— **阶段一（P0）✅ 预热已收官**（v10.37：命中 16,200/17,329，缓存 cached 16,312 / withReq 16,111）· **阶段二 2-B 已收口**（v10.34 字段校准 + **v10.35 口径决策已落**）· **2-A / 2-C 已开工并收口**（v10.36：唯一真源 + 代际护栏；剩余 C 类 145 / 36 条待下轮）· **派生索引已按预热结果重建**（v10.37：`data/spec-req.json` Steam 官方原文 3.9% → **94.1%**）。★ **v10.38 已把每日维护收进一条命令**（`node tools/daily-sync.js`：10 步 → 1 条，单轮实测 28 秒，只回吐 ≤8KB 摘要）；★ 预热**已跑完**（无需再跑）；★ `spec-req.json` 换数据后**必须重启服务**才生效（v10.37 实测：服务内存陈旧快照 `cached 4423` vs 磁盘 `16312`）。
