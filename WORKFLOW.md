...
```bash
# 第一层：静态（前置闸 2 个 + 33 套 / 2236 条 / 必须 0 失败）
...
| ① 静态 | `tools/run-all.js` | **33 套 / 2236 条** | 秒级、无需人盯 | 每次改完 |
| ② 行为 | `test-emulator-page.js`（jsdom，含在 33 套内） | 119 条 | 需服务在 8123 | 每次改完 |
...
**当前阶段**：`ROADMAP.md` 第二节 **阶段一（P0）· 配置要求预热** —— **已授权开工（v10.33）**：`node tools/build-steam-req.js --all --batch=500` 正在跑，待抓 **17,329** 条。★ 预热期间**线上就在生效**（v10.33 的跨进程可见性），跑完**不需要重启服务**；断点续跑。
