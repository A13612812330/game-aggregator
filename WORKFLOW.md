...
```bash
# 第一层：静态（前置闸 2 个 + 34 套 / 2299 条 / 必须 0 失败）
...
| ① 静态 | `tools/run-all.js` | **34 套 / 2299 条** | 秒级、无需人盯 | 每次改完 |
| ② 行为 | `test-emulator-page.js`（jsdom，含在 34 套内） | 119 条 | 需服务在 8123 | 每次改完 |
...
**当前阶段**：`ROADMAP.md` 第二节 —— **阶段一（P0）预热仍在跑**（v10.33）· **阶段二 2-B 已完成**（v10.34，只剩一处口径决策）。阶段一：**已授权开工（v10.33）**：`node tools/build-steam-req.js --all --batch=500` 正在跑，待抓 **17,329** 条。★ 预热期间**线上就在生效**（v10.33 的跨进程可见性），跑完**不需要重启服务**；断点续跑。
