...
```bash
# 第一层：静态（前置闸 2 个 + 36 套 / 2415 条 / 必须 0 失败）
...
| ① 静态 | `tools/run-all.js` | **36 套 / 2415 条** | 秒级、无需人盯 | 每次改完 |
...
**当前阶段**：`ROADMAP.md` 第二节 —— **阶段一（P0）预热仍在跑**（v10.33，已触发 HTTP 429 限流 ⇒ 剩余超过 7h）· **阶段二 2-B 已收口**（v10.34 字段校准 + **v10.35 口径决策已落**）· **2-A / 2-C 已开工并收口**（v10.36：唯一真源 + 代际护栏；剩余 C 类 145 / 36 条待下轮）。阶段一：**已授权开工（v10.33）**：`node tools/build-steam-req.js --all --batch=500` 正在跑，待抓 **17,329** 条。★ 预热期间**线上就在生效**（v10.33 的跨进程可见性），跑完**不需要重启服务**；断点续跑。
