...
| 层 | 工具 | 规模 | 特点 | 何时跑 |
|---|---|---|---|---|
| ① 静态 | `tools/run-all.js` | **21 套 / 1388 条** | 秒级、无需人盯 | 每次改完 |
| ② 行为 | `test-emulator-page.js`（jsdom，含在 21 套内） | 119 条 | 需服务在 8123 | 每次改完 |
...
## 六、当前规模基线（2026-09-18 实测，v10.22）

| 库 | 规模 | 接口 |
|---|---|---|
| 端游库 | **18,928** = XDGAME 15,319 + 机地 **17,220**（合并去重后） | `/api/library/stats` |
| **机地全量话题**（v10.22 新） | **17,220**（`jidi-topics.json`，18 页/32s/10.3MB） | `/api/jiditopics/stats` |
| **PC 要求库**（v10.22 新） | **16,575** 款（机地 ∪ Steam 按 appid join）· DX 覆盖 **8,312** | `/api/spec/dict` |
...
| Steam 官方配置 | **653** 款（要求库的 Steam 侧候选） | `/api/spec/dict` |

**接口总量 73 条路由**（v10.22 新增 `/api/download`、`/api/jiditopics/*`、`/api/spec/*`），
`server.js` **1,473 行**（v10.22 实测）。
...
| 项 | 内容 | 阻塞点 |
|---|---|---|
| **v10.23** | 解包匹配**字段校准** | 等用户给解包 JSON 样本。**只改 `data/spec-dict.js`**，界面与接口不动 |
...
| **★ 发布三坑（本轮一次全踩）** | ① **判据不是 HTTP 200** —— v2/join/v3 **三个域名全 200 却全是旧版**，只能比 `index.html` md5 ② **LIVE 不跟随本地改动**（放临时文件探远端 → 404）⇒ 它是**快照**，改完本地**不会**自动上线 ③ 未登记的平台域名**不稳**，v10.23 发布优先「新建 app」 |
