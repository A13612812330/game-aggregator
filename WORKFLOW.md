...
> 基线：**v10.20**（2026-09-18）· 服务 `localhost:8123`
...
| C | **发布**（覆盖线上） | 用户明确要求 | 攒够一版才发 | `gamehub-agg-v2.app.workbuddy.host` |
...
| 手游中心（合并索引） | **3,195** · 匹配端游 **1,530（47.9%）** · 双料 89 | `/api/mobilehub/stats` |
...
| 项 | 内容 | 阻塞点 |
|---|---|---|
| **v10.21** | 解包匹配**字段校准** | 等用户给解包 JSON 样本。**只改 `data/spec-dict.js`**，界面与接口不动 |
| **发布** | v10.19 + v10.20 **一起发** | 等校准完，用户已明确「校准后再一次发布」 |

### P0 · 数据层（决定所有上层功能的天花板）

| # | 项 | 现状 | 目标 | 为什么排第一 |
|---|---|---|---|---|
| **#33** | 联网补名提升端游命中率 | 手游中心匹配 **47.9%**（1,530/3,195） | 60%+ | ★ **一个指标卡住多个下游**：一半条目无法关联端游库 → 「点不进详情」「封面空」「双料只有 89 条」全都是它的下游症状 |
| **P0-1** | 扩端游库收录 | 机地仅 **66** 条 vs XDGAME 15,319 | 补齐机地全量话题 | 比例极不均衡，说明机地同步明显不足；且两库定位错位（手游库偏老游戏/小体积，端游库偏近两年 3A） |
...
| 解包匹配结果页 | 样本到手后可加「按可跑档位分组」「导出清单」 | 依赖 v10.21 校准 |
