...
> | **当前分享链接** | `https://gamehub-agg-v4.app.workbuddy.host/`（v10.23 起，**v10.24 发布后链接未变**；**已弃用**：`36aa37e9…` 停 v10.22〔未登记，发布工具够不着〕/ `gamehub-agg-v3` 停 v10.21 / `-v2` 停 v10.18 / `-join` 停 v10.17） |
> | 质量基线（当前是 **23 套 1522 条**） | `node tools/run-all.js`（一键跑，实测输出） |
>
> ★ **2026-09-20（v10.24）发布结论**：选了「**覆盖当前 LIVE**」`wbapp_047aLTlMY7YdDmtVpp3BYa`，
> **成功复用同一 sandbox** `445143a7b3004d749eab6be0fe8836e5` ⇒ **链接没变**。
> ⇒ v10.23 记的「覆盖旧 app 全部被硬拒 ⇒ 只能新建」**这次不成立** —— 改成
> 「**环境相关的偶发拒绝：先试覆盖，被拒再新建**」。
> ⚠️ 发布工具**要求用户选定目标**（4 个同名登记），不能自己挑；也**别用 sites 的 `unpublish`** 清旧 app（会误伤 LIVE）。
>
