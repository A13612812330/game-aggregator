...
> | **当前分享链接** | `https://36aa37e911e6447eb86eb187240daff2.app.workbuddy.host/`（v10.22 起） |
> | 每次收尾的五项状态 | `node tools/report.js`（**实测**，不靠记忆） |
>
> ⚠️ **下面 v10.10 那段里的分享链接是同一条域名，但当时的理由相反**（当时它被标成「已弃用的旧域名」）。
> 2026-09-18（v10.22）实测后**改成 LIVE**：v3 的发布环境已失效、发布工具硬拒绝覆盖
> （「预留域名未绑定到本次发布环境」），而这个域名上**确实是 v10.22**
> （三页逐字节一致 + `/api/jiditopics/stats` 17,220 条 + `/api/download` 通）。
> ⚠️ 它**未登记**在 `.workbuddy/applications.yaml`，且实测**不跟随本地改动（是快照）**
> ⇒ **v10.23 发布优先「新建 app」拿一个已登记的新链接**。
> **已弃用**：`gamehub-agg-v3`（停在 v10.21）/ `gamehub-agg-v2`（v10.18）/ `gamehub-agg-join`（v10.17）。
> ⚠️ 判定版本一律比 `index.html` 的 md5 —— 这四个域名**全部返回 200**，状态码区分不出新旧。
>
