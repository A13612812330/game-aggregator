 ...
 *
 * 覆盖四块（对应用户四条口径）：
 *   ① 版式顺序：封面+标题(吸顶) → 评分+参数表 → 介绍/版本 → 配置要求 → 手机模拟器配置
 *      → 修改器+云存档 → 同分类更多 → 本体/Mod/修改器 → 其他；操作按钮组固定抽屉底部
...
console.log('=== ① 详情页版式顺序 ===');
{
  ...
  /* ★ 反证要点：把 `${shotsHtml}` 挪回「同分类之后」（改造前的位置）本组立刻变红。
     下面这条是**位置本身**的判据：预览必须在「配置要求」之前。 */
  ok(TPL.indexOf('${shotsHtml}') < TPL.indexOf('id="reqSlot"'),
    '★ 游戏预览在「配置要求」之前（改造前它排在第 7 位，要翻一整屏才看到）');
  ok(TPL.indexOf('id="dlSlot"') > TPL.indexOf('id="relSlot"'),
    '★ 下载三专区在「同分类更多」**之后**（用户口径：本体/修改器/Mod 放在最下面）');
  ok(TPL.indexOf('id="crossSrc"') > TPL.indexOf('id="dlSlot"'),
    '「其他」块确实在三专区之后');
}
