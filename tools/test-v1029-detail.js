...
console.log('\n=== ③ 右侧悬浮定位条 ===');
{
  ...
  /* ⚠️ 门槛从 9 放到 8（v10.32）：修改器 / 云存档两块**并排在同一行**之后，
     定位条上把它们合并成一项（用户口径：「修改器和存档现在是一行了，所以只需要一个」）——
     两块的 `rect.top` 只差几像素，保留两项会**同时高亮两个序号**，点哪个都跳同一行。
     ⇒ 这是**预期变更**，不是分区丢了：判据改成守「≥ 8」+ 下面新增的合并项断言。
     ⚠️ 别再改回 `>= 9`：那等于要求把两个入口加回来。 */
  ok(n >= 8, '★ 定位条分区数 ≥ 8（v10.32 起修改器/云存档合并成一项）', n + ' 项');
  ok(/nm: '评分参数'/.test(rail), '★★ D_RAIL 里有「评分参数」这一项（用户反馈「好像没更新」的根因）');
  /* ★ v10.32：两块合并成一项，且该指向两块的**共同容器** —— 指回单个块会让另一块无处可归 */
  ok(/nm: '修改器\/云存档'/.test(rail), '★ v10.32：「修改器/云存档」是**一项**');
  ok(/sel: \['#trSvSlot'\]/.test(rail), '★ 它锚定到两块的共同容器 #trSvSlot（不是 #trBlock / #svBlock）');
  ok(!/nm: '修改器'/.test(rail), '★★ 反向断言：定位条上不再有单独的「修改器」项');
  ok(!/nm: '云存档'/.test(rail), '★★ 反向断言：定位条上不再有单独的「云存档」项');
