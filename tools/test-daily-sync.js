...
console.log('\n=== ③ ★ --dry 必须拦住每一个有写副作用的步骤 ===');
{
  ...
  eq(writes.length, 7, 'STEP_DEFS 恰好 7 个有写副作用的步骤（8 步里的 health 是只读）');
...
console.log('\n=== ⑥ 实现约束（防退回写死路径） ===');
{
  ...
  for (const f of ['isTaskDone', 'planStep', 'decideStatus', 'tally', 'buildSummary', 'fmtDelta', 'parseArgs', 'readProduct', 'sourceCounts']) {
    ok(typeof DS[f] === 'function', '导出可测函数 ' + f);
  }
  ok(Array.isArray(DS.STEP_DEFS) && DS.STEP_DEFS.length >= 8, '导出 STEP_DEFS（8 步）');
}
...
console.log('\n=== ⑦ 参数解析 ===');
{
  ...
  ok(q.t.xd > 0 && q.t.bh > 0 && q.t.jidi > 0 && q.t.script > 0 && q.t.net > 0,
    '各步有默认超时（按类别：xd / bh / jidi / script / net —— script 与 net 是**按性质**分组，不是按步骤名）');
}
