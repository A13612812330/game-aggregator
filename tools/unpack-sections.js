...
(function () {
  ...
  function card(it) {
    ...
    /* ★ v10.39：**游戏侧未声明的维度必须可见**（口径见 data/spec-match.js 的 judge）。
       病根：`skip` 既表示「配置未涉及」也表示「游戏没标」，而上面 chips 把所有 skip
       一并过滤掉 ⇒ 用户看到「流畅」，却看不出这个结论**没有图形接口依据**。
       实测全库 **6,585 款（38.5%）** 属此类（spec/dict 的 withDx 10,535 / 17,120）。
       ★ 与 uj 分开是刻意的：uj = 我方配置缺信息，us = 游戏数据没标注 ——
       责任侧不同，文案就不能一样，否则用户不知道该去补哪一边。 */
    const basisTxt = (it.basis || []).length
      ? ' —— 本款结论由' + it.basis.map((d) => DIM_NAME[d] || d).join('、') + '判定' : '';
    const us = (it.unstated || []).length
      ? '<span class="up-ch us" title="' + esc('游戏未标注：' + it.unstated.map((d) => DIM_NAME[d] || d).join('、') + basisTxt) + '">'
        + '未声明：' + it.unstated.map((d) => DIM_NAME[d] || d).join('、') + '</span>' : '';
    const min = [];
    ...
    return '<article class="emu-card' + (it.cover ? ' has-cov' : '') + ' up-mc" data-verdict="' + it.verdict + '" data-name="' + esc(it.name) + '">' +
      ...
      '<div class="bd">' +
        ...
        '<div class="up-chips">' + chips + us + uj + '</div>' +
  ...
  async function initDict() {
    try {
      ...
      $('upDict').innerHTML =
        ...
        /* ★ v10.39：新口径必须自述在这里 —— 否则用户在卡片上看到「未声明：图形接口」
           会以为是渲染 bug。规则同 data/spec-match.js 的 judge：游戏没标某项（如 DX），
           该维度不参与判定，结论按其余维度推定，**不因此翻成「待确认」**。 */
        '游戏没标某项（如 DirectX 要求）时，该维度<b>不参与判定</b>：卡片会挂「未声明：图形接口」提醒，' +
        '本款结论按其余维度推定，<b>不因此翻成「待确认」</b>。<br>' +
        srcLine + '</div>' +
        '</div>';
